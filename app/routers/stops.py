...
from ..normalize import departure_from_otp, route_ref, route_ref_from_db, stop_from_db, stop_from_otp
from ..otp import DEPARTURES_QUERY, STOP_QUERY
...
async def nearby(rt: CityRuntime = Depends(city_runtime), lat: float = Query(...), lon: float = Query(...),
    ...
    return {"stops": [{**stop_from_db(rt.city, dict(r)), "distanceMeters": round(r["dist"], 1)} for r in rows]}
...
async def stop_detail(stopId: str, rt: CityRuntime = Depends(city_runtime)):
    ...
    try:
        data = await rt.otp.graphql(STOP_QUERY, {"id": rt.city.scoped(stopId)})
        s = data.get("stop")
        if s:
            if base is None:
    ...
    if not routes:   # OTP down or stop without patterns: fall back to what the static ingest learned
        async with pool().acquire() as c:
            fv = await c.fetchval("SELECT id FROM feed_version WHERE city=$1 AND is_active LIMIT 1", rt.city.id)
            rows = await c.fetch(
                """SELECT r.* FROM stop_route sr JOIN route r ON r.feed_version_id=sr.feed_version_id
                     AND r.route_id=sr.route_id WHERE sr.feed_version_id=$1 AND sr.stop_id=$2
...
async def departures(stopId: str, rt: CityRuntime = Depends(city_runtime),
    ...
    data = await rt.otp.graphql(DEPARTURES_QUERY, {"id": rt.city.scoped(stopId), "n": limit, "range": minutes * 60})
    s = data.get("stop")
    if not s:
        raise StopNotFound(f"stop '{stopId}' not found")
    ...
    deps.sort(key=lambda d: d["realtimeTime"] or d["scheduledTime"])
