...
- `GET /v1/cities/{city}/geocode?q=portal%20norte&lat=&lon=&limit=8`
  ...
  - IDECA answers are cached in Postgres by normalised query (`geocode_cache`): a hit is reused for 180 days, a "not an address" for 7; when the upstream fails a stale hit still answers. `/health.geocoder.cache` = `{rows, hits, misses}`.
  - **Named areas** (`geocoder.areas`, Bogotá: Catastro's 1 231 sectores catastrales + 20 localidades, mirrored from their ArcGIS layers every `refreshDays`, table `place_area`): a result `{"id": "area:barrio:008102", "name": "Chico", "label": "Barrio · Chapinero", "type": "place", "source": "catastro"}`; label nouns follow `locale` ("Neighbourhood · Chapinero"). `/health.geocoder.areas` = `{enabled, barrios, localidades, updatedAt, ok, error}`. `POST /v1/admin/cities/{city}/ingest-places` (editor) mirrors them now.
...
- v2.2 adds the editable section **`geocoder`**: `{"photonUrl": string|null, "ideca": {"enabled": bool, "url": string, "apiKey": string|null, "aliases": {"avenida boyacá": "AK 72", ...}}, "areas": {"enabled": bool, "barriosUrl": string|null, "barriosNameField": "SCANOMBRE", "barriosCodeField": "SCACODIGO", "localidadesUrl": string|null, "localidadesNameField": "LOCNOMBRE", "localidadesCodeField": "LOCCODIGO", "refreshDays": 30}}`. `ideca.apiKey` follows the assistant key's rules: masked on read (`••••374c`), the mask echoed back keeps the stored key, omitted keeps it, `null` drops the override so the YAML/environment value (`IDECA_API_KEY`) applies again. It never appears in the public city payload. `/health` reports the provider under `geocoder.ideca` (`enabled`, `calls`, `failed`, `okRate`, `lastError`).
...
"Br. Chicó Norte II Sector") — the city's own area (`catastro`) before Photon's, and an exact place more than
20 km from the user / city centre is a namesake elsewhere, not the answer.
